...
class UsersController extends Controller
{
    ...
    function getExplorePageProfileList(Request $request)
    {
        ...
        $profiles = Users::with('images')->has('images')->whereNotIn('id', $blocked_users)->where('is_block', 0)->inRandomOrder()->limit(15)->get();
    ...
    {
        $verifyRequest = VerifyRequest::find($request->verification_id);
    ...
    function updateLikedProfile(Request $request)
    {
        ...
        $user = Users::find($request->user_id);
        $my_user = Users::find($request->my_user_id);
        ...
        $my_user->likedprofile = $request->profiles;
        $my_user->save();
        
        $checkUserNotification = UserNotification::where('user_id', $request->user_id)
        ...
        if ($checkUserNotification == null) {
            ...
            $userNotification->my_user_id = $my_user->id;
    ...
    function fetchLikedProfiles(Request $request)
    {
        ...
        $array = explode(',', $user->likedprofile);
    ...
    function updateProfile(Request $req)
    {
        ...
        if ($data == null) {
            ...
            $data = Users::where('id', $req->user_id)->with('images')->has('images')->first();

            return response()->json(['status' => true, 'message' => __('app.Updatesuccessful'), 'data' => $data]);
        }
    }
